...
LIBNAME NHIS    "\\userdata\obrienkm2\Documents\CARRIERS\OBrien project\USE_NHIS_2000_DATASETS";
LIBNAME LIBRARY "\\userdata\obrienkm2\Documents\CARRIERS\OBrien project\USE_NHIS_2000_DATASETS";

*FILENAME ASCIIDAT "\\userdata\obrienkm2\Documents\CARRIERS\OBrien project\USE_NHIS_2000_DATASETS\SAMADULT.DAT";

proc print data=asciidat;
run;
...
DATA NHIS.SAMADULT;
   * CREATE A SAS DATA SET;
   INFILE "\\userdata\obrienkm2\Documents\CARRIERS\OBrien project\USE_NHIS_2000_DATASETS\samadult.dat" PAD LRECL=2147;
...
DATA famhist;
	SET NHIS.SAMADULT;
	KEEP AGE_P ALCNDR12 BIRTHAGE BIRTHNUM BMI CANAGE5 
	FHDAGE5 FHDMAN5 FHMAGE5 FHSAGE5 FHSMAN5 HISCOD_I SMKEV RC_SMP_I
	FHMTYP5 FHSTYP5 FHDTYP5 SEX WTFA_SA CANEV CNKIND5 PSU PX STRATUM;
RUN; 
PROC FREQ data=famhist;
	TABLES CANEV CNKIND5 HISCOD_I RC_SMP_I;
RUN;

*limit to women age 18+;
DATA famhist;
	SET famhist;
	WHERE SEX=2 AND AGE_P>=18;
	IF FHSTYP5=1 | FHDTYP5=1 | FHMTYP5=1 THEN famhist=1;
		ELSE famhist=0;
	IF HISCOD_I=2 THEN raceeth="NHW";
		ELSE IF HISCOD_I=3 THEN raceeth="NHB";
		ELSE IF HISCOD_I=1 THEN raceeth="Hispanic";
		ELSE IF HISCOD_I=4 AND RC_SMP_I=4 THEN raceeth="Asian";
		ELSE IF HISCOD_I=4 AND RC_SMP_I in (3,5,6) THEN raceeth="Other";
	IF CNKIND5=1 THEN BCcase=1;
		ELSE IF CNKIND5~=1 THEN BCcase=0;
	IF 18<=AGE_P<25 THEN agegroup=0;
		ELSE IF 25<=AGE_P<30 THEN agegroup=1;
		ELSE IF 30<=AGE_P<35 THEN agegroup=2;
		ELSE IF 35<=AGE_P<40 THEN agegroup=3;
		ELSE IF 40<=AGE_P<45 THEN agegroup=4;
		ELSE IF 45<=AGE_P<50 THEN agegroup=5;
		ELSE IF 50<=AGE_P<55 THEN agegroup=6;
		ELSE IF 55<=AGE_P<60 THEN agegroup=7;
		ELSE IF 60<=AGE_P<65 THEN agegroup=8;
		ELSE IF 65<=AGE_P<70 THEN agegroup=9;
		ELSE IF 70<=AGE_P<75 THEN agegroup=10;
		ELSE IF 75<=AGE_P<80 THEN agegroup=11;
		ELSE IF 80<=AGE_P THEN agegroup=12;

		*before age 50;
		IF AGE_P<50 THEN agegroup50=0;
			ELSE IF AGE_P>=50 THEN agegroup50=1;
RUN;
PROC MEANS data=famhist;
	VAR AGE_P;
RUN; 

PROC FREQ data=famhist;
	TABLES famhist raceeth BCcase famhist*agegroup famhist*BCcase;
RUN;

*for supplementary tables;
PROC FREQ data=famhist;
	WEIGHT WTFA_SA;
	TABLES famhist raceeth BCcase famhist*agegroup famhist*raceeth famhist*BCcase;
RUN;

PROC FREQ data=famhist;
	WEIGHT WTFA_SA;
	TABLES famhist famhist*agegroup50;
RUN;
PROC FREQ data=famhist;
	WEIGHT WTFA_SA;
	TABLES raceeth*agegroup*famhist;
RUN;
